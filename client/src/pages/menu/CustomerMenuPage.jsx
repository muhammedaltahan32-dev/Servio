...
import { normalizeImageUrl } from "../../services/ApiService.js";
import {
  Cat_ID,
  Cat_Name,
  Menu_BaseImage,
...
export const CustomerMenuPage = () => {
  ...
  const activeCategory = React.useMemo(
    () => safeCategories.find((category) => category[Cat_ID] === selectedCategoryId) ?? safeCategories[0] ?? null,
  ...
  const galleryImages = React.useMemo(() => {
    ...
    const images = [selectedItem[Menu_BaseImage], ...(Array.isArray(selectedItem[Menu_Images]) ? selectedItem[Menu_Images] : [])];
        ...
        <Container maxWidth="lg">
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2} justifyContent="space-between" alignItems={{ xs: "flex-start", sm: "center" }}>
            ...
            <Button
              variant="contained"
              color="warning"
              onClick={() => setSelectedCategoryId((current) => current || safeCategories[0]?.[Cat_ID] || "")}
            >
              View menu
            </Button>
          </Stack>
            ...
            {safeCategories.length > 0 && (
              ...
              >
                {safeCategories.map((category) => (
                  <Tab
                    key={category[Cat_ID]}
                    label={category[Cat_Name] || "Category"}
                  ...
                  return (
                    ...
                    >
                      <CardActionArea onClick={() => setSelectedItem(item)} sx={{ height: "100%" }}>
                        <CardMedia
                          component="img"
                          image={image || "https://images.unsplash.com/photo-1544025162-d76694265947?auto=format&fit=crop&w=900&q=80"}
                          alt={item[Menu_Name]}
            ...
            <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 1 }}>
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  {safeCategories.find((category) => category[Cat_ID] === selectedItem[Menu_CatID])?.[Cat_Name] || "Menu item"}
                </Typography>
              </Box>
              ...
              >
                  ...
                  >
                    {galleryImages.length > 0 ? (
                      galleryImages.map((image, index) => (
                        ...
                        >
                          <CardMedia component="img" image={image} alt={`${selectedItem[Menu_Name]} ${index + 1}`} sx={{ height: 190, objectFit: "cover" }} />
                        </Card>
                      ))
                    ) : (
                ...
                <Box>
                  ...
                  <Typography variant="body1" color="text.secondary" sx={{ lineHeight: 1.8, whiteSpace: "pre-line" }}>
                    {selectedItem[Menu_Descriptions] || "Fresh ingredients, balanced flavors, and a warm presentation make this dish a favorite choice for guests."}
                  </Typography>
                </Box>
              </Box>
            </DialogContent>
